...
import { Plus, Trash2, Search, Pencil, GripVertical } from "lucide-react";
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {grouped.map((col) => {
          ...
          <div key={col.value} className="space-y-3 rounded-2xl bg-muted/40 p-3">
            ...
            <div
              className={`min-h-32 space-y-2 rounded-xl p-1 transition-colors ${dragOverCol === col.value ? "bg-primary/10 ring-2 ring-primary/40" : ""}`}
              onDragOver={(e) => {
                e.preventDefault();
              ...
              onDrop={() => {
                ...
                if (task && task.status !== col.value) updateStatus.mutate({ id: task.id, status: col.value });
                setDraggingId(null);
                setDragOverCol(null);
              }}
            >
              {isLoading &&
                Array.from({ length: 2 }).map((_, i) => <Skeleton key={`sk-${i}`} className="h-24 w-full rounded-xl" />)}
              {!isLoading &&
                col.items.map((t) => {
                  ...
                  return (
                    <Card
                      ...
                      className={`gap-2 p-3 transition-all hover:shadow-md cursor-grab active:cursor-grabbing ${draggingId === t.id ? "opacity-50 ring-2 ring-primary/40" : ""}`}
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <span className={`whitespace-nowrap rounded-full border px-2 py-0.5 text-[10px] font-medium ${prio.cls}`}>{prio.label}</span>
                      </div>
              ...
              {!isLoading && col.items.length === 0 && (
                <p className="py-6 text-center text-xs italic text-muted-foreground">Solte tarefas aqui</p>
              )}
            </div>
          </div>
          );
